...
import { ArrowRight, Clock, Zap, BookOpen, Lightbulb } from "lucide-react";
...
export default function NextBestAction({ action, estimatedTime, impact, reason, whyRecommends, route }: NBA) {
  ...
  return (
    <motion.div
      ...
      style={{ background: "linear-gradient(135deg, #0D1220 0%, rgba(129,140,248,0.05) 100%)", border: "1px solid rgba(129,140,248,0.22)", boxShadow: "0 0 50px rgba(129,140,248,0.05)" }}>
      ...
      <div className="relative z-10 flex flex-col gap-5">
        ...
        <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(1.25rem, 3.2vw, 1.65rem)", color: "var(--text-prime)", lineHeight: 1.35, fontWeight: 600 }}>
          {action}
        </p>
        ...
        <div className="flex flex-wrap gap-3">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg"
            ...
            <span className="text-xs font-medium" style={{ fontFamily: "'DM Sans', sans-serif", color: "var(--text-muted)" }}>{estimatedTime}</span>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg" style={{ background: bg, border: `1px solid ${color}25` }}>
            ...
            <span className="text-xs font-semibold" style={{ fontFamily: "'DM Sans', sans-serif", color }}>{impact} impact</span>
          </div>
        </div>
        ...
        {/* Why Mina recommends this */}
        <div className="flex items-start gap-3 px-4 py-3.5 rounded-xl"
          style={{ background: "rgba(129,140,248,0.06)", border: "1px solid rgba(129,140,248,0.14)" }}>
          <Lightbulb size={14} className="mt-0.5 flex-shrink-0" style={{ color: "#818CF8" }} />
