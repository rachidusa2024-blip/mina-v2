...
export default function MinaInsightHero({
  ...
  coachHeader, coachLines, coachClosing, coachFocus,
  ...
  return (
    <motion.div
      ...
      style={{ background: "linear-gradient(135deg, #0D1120 0%, #111827 55%, rgba(0,201,167,0.04) 100%)", border: "1px solid rgba(0,201,167,0.2)", boxShadow: "0 0 80px rgba(0,201,167,0.06), 0 20px 60px rgba(0,0,0,0.4)" }}>
      ...
      <div className="relative z-10 p-7 sm:p-8 flex flex-col gap-7">
        ...
        {/* Structured coach voice */}
        <div className="flex flex-col gap-4">
          <p className="text-xs font-bold uppercase tracking-wider"
            style={{ fontFamily: "'DM Sans', sans-serif", color: "rgba(136,146,164,0.6)" }}>
            {coachHeader}
          </p>
          <div className="flex flex-col gap-3">
            {coachLines.map((line, i) => (
              <motion.p key={i}
                ...
                style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(1.05rem, 2.6vw, 1.32rem)", color: "var(--text-prime)", lineHeight: 1.55 }}>
                {line}
              </motion.p>
            ))}
          </div>
          {/* Closing — the "that means" line */}
          <motion.p
            initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.55 }}
            style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(1.05rem, 2.6vw, 1.32rem)", color: "var(--teal)", lineHeight: 1.55, fontStyle: "italic" }}>
            {coachClosing}
          </motion.p>
        </div>
        ...
        <div className="flex items-start gap-3 px-4 py-3.5 rounded-xl"
          ...
          <div className="w-1 self-stretch rounded-full flex-shrink-0 mt-0.5" style={{ background: "var(--teal)", minHeight: "16px" }} />
          <div className="flex flex-col gap-0.5">
            <span className="text-xs font-bold uppercase tracking-wider"
              style={{ fontFamily: "'DM Sans', sans-serif", color: "var(--teal)" }}>
              Today's focus
            </span>
            <p className="text-sm leading-relaxed" style={{ fontFamily: "'DM Sans', sans-serif", color: "var(--text-prime)" }}>
              {coachFocus}
            </p>
          </div>
        </div>
        ...
        {/* CTA row */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 pt-1" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <p className="text-sm leading-relaxed flex-1" style={{ fontFamily: "'DM Sans', sans-serif", color: "var(--text-muted)" }}>
            {recommendation}
          </p>
          <Link href={recommendedRoute}
            ...
            Continue next step <ArrowRight size={14} className="transition-transform group-hover:translate-x-0.5" />
          </Link>
        </div>
        ...
        <div className="flex flex-wrap gap-x-5 gap-y-1.5 pt-1" style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
          {TRUST.map(t => (
            <div key={t} className="flex items-center gap-1.5">
              <Check size={11} style={{ color: "rgba(0,201,167,0.55)", flexShrink: 0 }} />
              <span className="text-xs" style={{ fontFamily: "'DM Sans', sans-serif", color: "rgba(136,146,164,0.5)" }}>{t}</span>
            </div>
          ))}
        </div>
